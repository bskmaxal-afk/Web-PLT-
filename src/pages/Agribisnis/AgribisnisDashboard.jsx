...
export default function AgribisnisDashboard() {
  const navigate = useNavigate();
  const { setSelectedLaboratory, laboratories } = useContext(AppContext);

  // Filter laboratories to only show Agribisnis labs
  const agribisnisLaboratories = useMemo(() => {
    return (laboratories || []).filter((l) => {
      const p = (l.prodi || "").toLowerCase();
      const j = (l.jenisLab || "").toLowerCase();
      return p.includes("agribisnis") || j.includes("agribisnis");
    });
  }, [laboratories]);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'AGRIBISNIS UIN Jakarta - Laboratorium Agribisnis',
        text: 'Akses berbagai ruang laboratorium agribisnis untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran Anda.',
  ...
  const handleSelectLaboratory = (lab) => {
    ...
    navigate("/booking?rumpun=agribisnis");
  };
  ...
  const renderCard = (lab) => (
    ...
    >
      {/* Icon */}
      <div className={`lab-icon-container ${lab.colorClass || "bg-lime-500"}`}>
        {renderIcon(lab.icon)}
      </div>
      ...
      <div className="lab-card-action">
        <button
          ...
          className={`lab-action-btn ${lab.borderClass || "border-lime-500/20 hover:border-lime-500"} ${lab.textClass || "text-lime-600"} ${lab.hoverBtnClass || "hover:bg-lime-500 hover:text-white"}`}
        >
  ...
  return (
    <DashboardLayout>
      {/* ====== HERO BANNER (AGRIBISNIS CUSTOMIZED) ====== */}
      <section className="hero-section">
        ...
        >
          ...
          <div className="hero-content">
            <h2 className="hero-title">
              PLT Agribisnis UIN Jakarta
            </h2>
            <h3 className="hero-subtitle">
              Rumpun Laboratorium Agribisnis
            </h3>
            <p className="hero-description">
              Akses berbagai fasilitas laboratorium agribisnis untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran dengan peralatan berstandar tinggi.
            </p>
          </div>
        </div>
        ...
        <div className="hero-stats-card">
          ...
          <div className="hero-stat-item">
            <div className="hero-stat-icon-wrapper bg-lime-50 text-lime-600">
              <DoorOpen size={20} />
            </div>
            <div className="text-left">
              <div className="hero-stat-value">{agribisnisLaboratories.length}</div>
              <div className="hero-stat-label">Lab Agribisnis</div>
            </div>
          </div>
      ...
      <section className="labs-section">
        ...
        <div className="labs-header">
          <span className="labs-section-tag">
            — Pilih Ruang Laboratorium Agribisnis —
          </span>
          <h3 className="labs-section-title">
            Silahkan Pilih Ruang Laboratorium Agribisnis
          </h3>
          <p className="labs-section-subtitle">
            Pilih ruang laboratorium agribisnis yang akan digunakan sesuai kebutuhan Anda
          </p>
        </div>

        {/* Lab Cards Grid */}
        <div className="labs-grid">
          {agribisnisLaboratories.map((lab) => renderCard(lab))}
        </div>
      </section>
    </DashboardLayout>
  );
