...
export default function TambangDashboard() {
  const navigate = useNavigate();
  const { setSelectedLaboratory, laboratories } = useContext(AppContext);

  // Filter laboratories to only show Tambang labs
  const tambangLaboratories = useMemo(() => {
    return (laboratories || []).filter((l) => {
      const p = (l.prodi || "").toLowerCase();
      const j = (l.jenisLab || "").toLowerCase();
      return p.includes("tambang") || j.includes("tambang");
    });
  }, [laboratories]);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'TAMBANG UIN Jakarta - Laboratorium Tambang',
        text: 'Akses berbagai ruang laboratorium tambang untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran Anda.',
  ...
  const renderCard = (lab) => (
    ...
    >
      {/* Icon */}
      <div className={`lab-icon-container ${lab.colorClass || "bg-amber-500"}`}>
        {renderIcon(lab.icon)}
      </div>
      ...
      <div className="lab-card-action">
        <button
          ...
          className={`lab-action-btn ${lab.borderClass || "border-amber-500/20 hover:border-amber-500"} ${lab.textClass || "text-amber-600"} ${lab.hoverBtnClass || "hover:bg-amber-500 hover:text-white"}`}
        >
  ...
  return (
    <DashboardLayout>
      {/* ====== HERO BANNER (TAMBANG CUSTOMIZED) ====== */}
      <section className="hero-section">
        ...
        >
          ...
          <div className="hero-content">
            <h2 className="hero-title">
              PLT Tambang UIN Jakarta
            </h2>
            <h3 className="hero-subtitle">
              Rumpun Laboratorium Tambang
            </h3>
            <p className="hero-description">
              Akses berbagai fasilitas laboratorium tambang untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran dengan peralatan berstandar tinggi.
            </p>
          </div>
        </div>
        ...
        <div className="hero-stats-card">
          ...
          <div className="hero-stat-item">
            <div className="hero-stat-icon-wrapper bg-amber-50 text-amber-600">
              <DoorOpen size={20} />
            </div>
            <div className="text-left">
              <div className="hero-stat-value">{tambangLaboratories.length}</div>
              <div className="hero-stat-label">Lab Tambang</div>
            </div>
          </div>
      ...
      <section className="labs-section">
        ...
        <div className="labs-header">
          <span className="labs-section-tag">
            — Pilih Ruang Laboratorium Tambang —
          </span>
          <h3 className="labs-section-title">
            Silahkan Pilih Ruang Laboratorium Tambang
          </h3>
          <p className="labs-section-subtitle">
            Pilih ruang laboratorium tambang yang akan digunakan sesuai kebutuhan Anda
          </p>
        </div>

        {/* Lab Cards Grid */}
        <div className="labs-grid">
          {tambangLaboratories.map((lab) => renderCard(lab))}
        </div>
      </section>
    </DashboardLayout>
  );
