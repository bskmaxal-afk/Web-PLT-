...
export default function LingkunganDashboard() {
  const navigate = useNavigate();
  const { setSelectedLaboratory, laboratories } = useContext(AppContext);

  // Filter laboratories to only show Lingkungan labs
  const lingkunganLaboratories = useMemo(() => {
    return (laboratories || []).filter((l) => {
      const name = (l.name || "").toLowerCase();
      return name.includes("lingkungan");
    });
  }, [laboratories]);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'LINGKUNGAN UIN Jakarta - Laboratorium Lingkungan',
        text: 'Akses berbagai ruang laboratorium lingkungan untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran Anda.',
  ...
  const handleSelectLaboratory = (lab) => {
    ...
    navigate("/booking?rumpun=lingkungan");
  };
  ...
  const renderCard = (lab) => (
    ...
    >
      {/* Icon */}
      <div className={`lab-icon-container ${lab.colorClass || "bg-blue-500"}`}>
        {renderIcon(lab.icon)}
      </div>
      ...
      <div className="lab-card-action">
        <button
          ...
          className={`lab-action-btn ${lab.borderClass || "border-blue-500/20 hover:border-blue-500"} ${lab.textClass || "text-blue-600"} ${lab.hoverBtnClass || "hover:bg-blue-500 hover:text-white"}`}
        >
  ...
  return (
    <DashboardLayout>
      {/* ====== HERO BANNER (LINGKUNGAN CUSTOMIZED) ====== */}
        ...
        >
          ...
          <div className="hero-content">
            <h2 className="hero-title">
              PLT Lingkungan UIN Jakarta
            </h2>
            <h3 className="hero-subtitle">
              Rumpun Laboratorium Lingkungan
            </h3>
            <p className="hero-description">
              Akses berbagai fasilitas laboratorium lingkungan untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran dengan peralatan berstandar tinggi.
            </p>
          </div>
        </div>
        ...
        <div className="hero-stats-card">
          ...
          <div className="hero-stat-item">
            ...
            <div className="text-left">
              <div className="hero-stat-value">{lingkunganLaboratories.length}</div>
              <div className="hero-stat-label">Lab Lingkungan</div>
            </div>
          </div>
      ...
      <section className="labs-section">
        ...
        <div className="labs-header">
          <span className="labs-section-tag">
            — Pilih Ruang Laboratorium Lingkungan —
          </span>
          <h3 className="labs-section-title">
            Silahkan Pilih Ruang Laboratorium Lingkungan
          </h3>
          <p className="labs-section-subtitle">
            Pilih ruang laboratorium lingkungan yang akan digunakan sesuai kebutuhan Anda
          </p>
        </div>

        {/* Lab Cards Grid */}
        <div className="labs-grid">
          {lingkunganLaboratories.map((lab) => renderCard(lab))}
        </div>
      </section>
    </DashboardLayout>
  );
