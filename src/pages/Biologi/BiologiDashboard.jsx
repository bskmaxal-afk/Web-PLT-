...
export default function BiologiDashboard() {
  const navigate = useNavigate();
  const { setSelectedLaboratory, laboratories } = useContext(AppContext);

  // Filter laboratories to only show Biologi rumpun labs: Fisiologi, Mikrobiologi, Parasitologi
  const biologiLaboratories = useMemo(() => {
    const allowedNames = [
      "fisiologi",
      "mikrobiologi",
      "parasitologi"
    ...
    return (laboratories || []).filter((l) => {
      ...
      return p.includes("biologi") && allowedNames.some(allowed => name.includes(allowed));
    });
  }, [laboratories]);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'BIOLOGI UIN Jakarta - Laboratorium Biologi',
        text: 'Akses berbagai ruang laboratorium biologi untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran Anda.',
  ...
  const handleSelectLaboratory = (lab) => {
    ...
    navigate("/booking?rumpun=biologi");
  };
  ...
  const renderCard = (lab) => (
    ...
    >
      {/* Icon */}
      <div className={`lab-icon-container ${lab.colorClass || "bg-green-500"}`}>
        {renderIcon(lab.icon)}
      </div>
      ...
      <div className="lab-card-action">
        <button
          ...
          className={`lab-action-btn ${lab.borderClass || "border-green-500/20 hover:border-green-500"} ${lab.textClass || "text-green-600"} ${lab.hoverBtnClass || "hover:bg-green-500 hover:text-white"}`}
        >
  ...
  return (
    <DashboardLayout>
      {/* ====== HERO BANNER (BIOLOGI CUSTOMIZED) ====== */}
      <section className="hero-section">
        ...
        >
          ...
          <div className="hero-content">
            <h2 className="hero-title">
              PLT Biologi UIN Jakarta
            </h2>
            <h3 className="hero-subtitle">
              Rumpun Laboratorium Biologi
            </h3>
            <p className="hero-description">
              Akses berbagai fasilitas laboratorium biologi untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran dengan peralatan berstandar tinggi.
            </p>
          </div>
        </div>
        ...
        <div className="hero-stats-card">
          ...
          <div className="hero-stat-item">
            <div className="hero-stat-icon-wrapper bg-green-50 text-green-600">
              <DoorOpen size={20} />
            </div>
            <div className="text-left">
              <div className="hero-stat-value">{biologiLaboratories.length}</div>
              <div className="hero-stat-label">Lab Biologi</div>
            </div>
          </div>
      ...
      <section className="labs-section">
        ...
        <div className="labs-header">
          <span className="labs-section-tag">
            — Pilih Ruang Laboratorium Biologi —
          </span>
          <h3 className="labs-section-title">
            Silahkan Pilih Ruang Laboratorium Biologi
          </h3>
          <p className="labs-section-subtitle">
            Pilih ruang laboratorium biologi yang akan digunakan sesuai kebutuhan Anda
          </p>
        </div>

        {/* Lab Cards Grid */}
        <div className="labs-grid">
          {biologiLaboratories.map((lab) => renderCard(lab))}
        </div>
      </section>
    </DashboardLayout>
  );
