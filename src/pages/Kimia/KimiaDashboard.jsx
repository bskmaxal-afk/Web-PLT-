...
export default function KimiaDashboard() {
  ...
  const kimiaLaboratories = useMemo(() => {
    const allowedNames = [
      "kimia analitik",
      "kimia anorganik",
      "kimia pangan",
      "laboratorium penelitian",
      "laboratium penelitian"
    ];
    return (laboratories || []).filter((l) => {
      const name = (l.name || "").toLowerCase();
      return getRumpun(l) === "KIMIA" && allowedNames.some(allowed => name.includes(allowed));
    });
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: 'KIMIA UIN Jakarta - Laboratorium Kimia',
        text: 'Akses berbagai ruang laboratorium kimia untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran Anda.',
  ...
  const renderCard = (lab) => (
    ...
    >
      {/* Icon */}
      <div className={`lab-icon-container ${lab.colorClass || "bg-emerald-500"}`}>
        {renderIcon(lab.icon)}
      </div>
      ...
      <div className="lab-card-action">
        <button
          ...
          className={`lab-action-btn ${lab.borderClass || "border-emerald-500/20 hover:border-emerald-500"} ${lab.textClass || "text-emerald-600"} ${lab.hoverBtnClass || "hover:bg-emerald-500 hover:text-white"}`}
        >
        ...
        >
          ...
          <div className="hero-content">
            <h2 className="hero-title">
              PLT Kimia UIN Jakarta
            </h2>
            <h3 className="hero-subtitle">
              Rumpun Laboratorium Kimia
            </h3>
            <p className="hero-description">
              Akses berbagai fasilitas laboratorium kimia untuk mendukung kegiatan praktikum, penelitian, dan pembelajaran dengan peralatan berstandar tinggi.
            </p>
          </div>
        </div>
        ...
        <div className="hero-stats-card">
          ...
          <div className="hero-stat-item">
            ...
            <div className="text-left">
              <div className="hero-stat-value">{kimiaLaboratories.length}</div>
              <div className="hero-stat-label">Lab Kimia</div>
            </div>
          </div>
      ...
      <section className="labs-section">
        ...
        <div className="labs-header">
          <span className="labs-section-tag">
            — Pilih Ruang Laboratorium Kimia —
          </span>
          <h3 className="labs-section-title">
            Silahkan Pilih Ruang Laboratorium Kimia
          </h3>
          <p className="labs-section-subtitle">
            Pilih ruang laboratorium kimia yang akan digunakan sesuai kebutuhan Anda
          </p>
        </div>

        {/* Lab Cards Grid */}
        <div className="labs-grid">
          {kimiaLaboratories.map((lab) => renderCard(lab))}
        </div>
      </section>
    </DashboardLayout>
  );
